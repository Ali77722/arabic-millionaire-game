const CACHE_NAME='millionaire-ar-v4';
const ASSETS=[
...
self.addEventListener('fetch',e=>{
  ...
  if(e.request.destination==='script' || url.pathname.endsWith('.js')){
  ...
  e.respondWith(caches.match(e.request).then(cached=>cached||fetch(e.request).then(res=>{const copy=res.clone();caches.open(CACHE_NAME).then(c=>c.put(e.request,copy)).catch(()=>{});return res}).catch(()=>e.request.mode==='navigate'?caches.match('./index.html'):Response.error())));
});
